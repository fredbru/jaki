...
class DQN:
    ...
    def create_model(self):
        model = Sequential()
        #state_shape = [16,32]
        model.add(Dense(32, input_shape=(16,32), activation="relu")) # does this need to be 512? size of state space? flatten model?
    ...
    def act(self, state):
        ...
        if np.random.random() < self.epsilon:
            ...
            prediction = self.model.predict(np.expand_dims(state, 0))[0,:,:]
    ...
    def replay(self): # need to refit this function to use new adapted reward calculation?
        ...
        for sample in samples:
            ...
            target = self.target_model.predict(np.expand_dims(currentState, 0))
            # print(target[0].shape, 'shape') # target is an action. target[0] = 16x32
            y,z  = np.unravel_index(np.argmax(target[0], axis=None), target[0].shape)
            if done:
                target[0] = reward
            else:
                Q_future = max(self.target_model.predict(np.expand_dims(newState, 0)))
                # print(Q_future)
                target[0] = reward + Q_future * self.gamma
            # print(target.shape)
            self.model.fit(np.expand_dims(currentState, 0), target, epochs=1, verbose=0) #fit updates model weights
...
for trial in range(trials):
    ...
    groove = oneBarGrooves[np.random.randint(0,6000),:,:]
    # print(LSTM.summary())
    ...
    for step in range(trial_len):
        ...
        if done:
            print("1st", convertOneHotToList(groove))
